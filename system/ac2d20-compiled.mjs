...
const SYSTEM_NAME$1 = "Achtung! Cthulhu 2d20";
...
class MomentumTrackerV2
	...
	async #onChange(event, form, formData) {
		...
		const gmMomentum = game.settings.get(SYSTEM_ID$1, "gmMomentum");
		const partyMomentum = game.settings.get(SYSTEM_ID$1, "partyMomentum");
	...
	static async changeCounter(diff, type) {
		...
		const newValue = game.settings.get(SYSTEM_ID$1, type) + diff;
	...
	static async setCounter(value, type) {
		if (!game.user.isGM) {
			game.socket.emit(`system.${SYSTEM_ID$1}`, {
				type: "setCounter",
				payload: {value, type},
			});
			return;
		...
		if (type === "partyMomentum") {
			value = Math.min(game.settings.get(SYSTEM_ID$1, "maxMomentum"), value);
		}
		else {
		...
		game.socket.emit(`system.${SYSTEM_ID$1}`, {type: "updateCounter"});
	}
	...
	async _prepareContext(options={}) {
		...
		if (this._firstRun) {
			...
			await game.settings.set(
				SYSTEM_ID$1, "partyMomentum",
				Math.min(
					game.settings.get(SYSTEM_ID$1, "maxMomentum"),
					game.settings.get(SYSTEM_ID$1, "partyMomentum")
				)
			);
		}

		context.gmMomentum = game.settings.get(SYSTEM_ID$1, "gmMomentum");
		context.isGM = game.user.isGM;
		context.maxMomentum = game.settings.get(SYSTEM_ID$1, "maxMomentum");
		context.partyMomentum = game.settings.get(SYSTEM_ID$1, "partyMomentum");
...
var apps = /*#__PURE__*/Object.freeze({
	__proto__: null,
	CombatTracker2d20V2: CombatTracker2d20V2,
	MomentumTrackerV2: MomentumTrackerV2
});
 ...
 */
class ACNPCSheet extends ACActorSheet {
 ...
 * @extends {ACActorSheet}
 */
class ACVehicleSheet extends ACActorSheet {
...
class DieACChallenge extends foundry.dice.terms.Die {
...
class Update_240218_1 extends ACUpdateBase {
	static version = 240218.1;
}

class Update_240526_1 extends ACUpdateBase {
...
class Update_240526_2 extends ACUpdateBase {
...
class Update_240527_1 extends ACUpdateBase {
...
class Update_240529_1 extends ACUpdateBase {
...
class Update_240530_1 extends ACUpdateBase {
...
class Update_240531_1 extends ACUpdateBase {
...
class Update_240531_2 extends ACUpdateBase {
...
function registerSocketEvents() {
	game.socket.on(`system.${SYSTEM_ID}`, event => {
		if (event.type === "setCounter" && game.user.isGM) {
			cc2d20.apps.MomentumTrackerV2.setCounter(
				event.payload.value,
				event.payload.type
			);
		}
...
class ACChat {
	...
	static async onRenderChatMessage(message, html, data) {
		cc2d20.logger.debug("Running renderChatMessageHTML hook");
		...
		html.querySelectorAll(".add-button").forEach(element => {
			element.addEventListener("click", async ev => {
				...
				const actorId = cc2d20Roll.actorId;
				const itemId = cc2d20Roll.itemId;
